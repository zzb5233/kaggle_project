...
def model_factory(n_inputs):
    model = Sequential()
    model.add(Dense(800, input_shape=(n_inputs,)))
    model.add(PReLU(input_shape=(800, )))
...
X_train, y_train, _ = load_data("./training.csv")
X_test, _, id = load_data("./test.csv", output_y=False)
